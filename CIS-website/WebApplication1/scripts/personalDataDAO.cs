...
namespace WebApplication1.scripts
{
    public class personalDataDAO : personalDataDAOinterface
    {
        ...
        public void insertData(personalData data)
        {
            ...
            myConnection.Open();
            SqlCommand myCommand = new SqlCommand();
            myCommand.CommandText = "insert into personalData (firstName,lastName,mobile,gender,birthday,height,weight) Values ('" + data.getFname() + "','" + data.getLname() + "','" + data.getMobile() + "','" + data.getGender() + "','" + data.getBirthday() + "','" + data.getHeight().ToString() + "','" + data.getWeight().ToString() + "')  ;  ";
            myCommand.Connection = myConnection;
        ...
        public void updateData(personalData data)
        {
            ...
            myConnection.Open();
            SqlCommand myCommand = new SqlCommand();
            myCommand.CommandText = "UPDATE personalData SET firstName ='" + data.getFname() + "',lastName ='" + data.getLname() + "', mobile ='" + data.getMobile() + "', gender = '" + data.getGender() + "', birthday = '" + data.getBirthday() + "', height = '" + data.getHeight().ToString() + "', weight = '" + data.getWeight().ToString() + "' WHERE id = " + data.getId() + ";";
            myCommand.Connection = myConnection;
            myCommand.ExecuteNonQuery();
            myConnection.Close();
        }
